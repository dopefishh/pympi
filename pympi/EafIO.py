...
def parseEaf(filePath, eafObj):
	"""Parses the file given by path to the object given"""
	treeRoot = ElementTree.parse(filePath).getroot()
	eafObj.annotationDocument.update(treeRoot.attrib)
	del(eafObj.annotationDocument['{http://www.w3.org/2001/XMLSchema-instance}noNamespaceSchemaLocation'])
	tierNumber = 0
	for elem in treeRoot:
		if elem.tag == 'HEADER':
			...
			for elem1 in elem:
				...
				eafObj.timeslots[elem1.attrib['TIME_SLOT_ID']] = int(elem1.attrib['TIME_VALUE'])
		elif elem.tag == 'TIER':
			...
			eafObj.linguistic_types[elem.attrib['LINGUISTIC_TYPE_ID']] = elem.attrib
		elif elem.tag == 'LOCALE':
			eafObj.locales.append(elem.attrib)
		elif elem.tag == 'CONSTRAINT':
			eafObj.constraints[elem.attrib['STEREOTYPE']] = elem.attrib['DESCRIPTION']
...
def toEaf(filePath, eafObj, pretty=True):
	"""Writes the object given to the filepath given with or without pretty indentation"""
	rmNone = lambda x: dict((k, unicode(v)) for k, v in x.iteritems() if v is not None)
	ANNOTATION_DOCUMENT = ElementTree.Element('ANNOTATION_DOCUMENT', eafObj.annotationDocument)
	...
	for t in eafObj.tiers.iteritems():
		...
		for a in t[1][0].iteritems():
			...
			ElementTree.SubElement(alan, 'ANNOTATION_VALUE').text = unicode(a[1][2])
		for a in t[1][1].iteritems():
			...
			ElementTree.SubElement(rean, 'ANNOTATION_VALUE').text = unicode(a[1][1])
	...
	for l in eafObj.controlled_vocabularies.iteritems():
		...
		for c in l[1][1].itervalues():
			ElementTree.SubElement(cv, 'CV_ENTRY', rmNone(c[0])).text = unicode(c[1])
